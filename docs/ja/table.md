...
## 権限
そのテーブルに対してアクセスを許可したいユーザー・組織を、該当する権限の項目に追加してください。

![カスタムテーブル画面](img/table/table_authority.png)
  
- 追加されたユーザー・組織は、該当する権限でテーブルに対してアクセスが可能になります。  
- ただし、メニューの「システム設定」で登録しているユーザー・組織も、テーブルに対してアクセスが可能です。  
- 上記どちらにおいても権限が設定されていないユーザー・組織は、そのテーブルにはアクセスできません。  

※権限の設定に関して、詳細はこちら[役割・権限](/ja/permission.md)をご参照ください。
...
- 次に編集画面右上にある「削除」リンクをクリックしてください。  

![カスタムテーブル画面](img/table/table_delete.png)  

- 下記画像のように確認画面が表示されるので、テキストエリアにdelete meと入力し、確認ボタンをクリックしてください。テーブルの削除が行われます。  
...
その場合、カスタムテーブルの拡張設定画面で、ラベルとして表示したい列に対し、以下のように設定を行います。  
...
### データ自動共有設定
データ保存時、列の値として設定されている組織またはユーザーに、自動的にデータを共有することができます。以下のような用途でご利用ください。  
- データ「契約情報」の新規作成時に、列「主担当者」に割り振れたユーザーに、自動的に編集権限を追加する。  
- データ「契約情報」の更新時に、列「関係組織」に割り振れた組織に、自動的に閲覧権限を追加する。  
...
ユーザー「user1」に、この契約情報を、編集権限で共有します。
...
#### 注意
トリガーが「更新時」の時、対象のユーザー・組織の値の変更を行っても、元々設定されていたユーザー・組織の共有情報は削除されず、共有されたままになります。

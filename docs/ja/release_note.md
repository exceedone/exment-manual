...
## v6.2.8 (2026/01/22)
1. 機能修正
    - 親子テーブルで親テーブルが閲覧権限しかない場合、子テーブルの編集権限を持っていても編集できるように修正
    - カスタムデータ取得APIに親データの取得オプションを追加
1. 不具合修正
    - 集計ビューのグループ化・日付型でソート時に順番が正しくない不具合修正
    - 通知設定で実施アクションがEメールだとエラーになる不具合修正
    - レコードの表示画面からデータ一覧画面に戻った際に画面位置がレコードの位置に戻るようにしたい

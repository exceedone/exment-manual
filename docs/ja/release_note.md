...
## v6.2.11 (2026/06/19)
1. 不具合修正
    - v6.2.10 リリース後、バージョン表示不具合の修正
    - テーブルコピー時にバーコード関連の設定がコピーされてしまう不具合修正
    - テンプレートをエクスポート⇒別の環境にインポートすると列やフォームの設定が正しく反映されない不具合修正
    - サムネイル付きのテンプレートを読み込むとExceptionになる不具合修正
    - テンプレートは全設定をエクスポート->インポートできるように修正
    - インストール後の初期設定画面でユーザーテンプレートを削除するとダイアログが閉じない不具合修正
    
## v6.2.10 (2026/06/11)
1. 機能追加
    - ログ自動削除　詳細は[こちら](/ja/logs)
...
1. 機能修正
    - MySQL 8.4対応
1. 不具合対応  
    - 「見出し表示方法」に「縦並び」を選択すると 編集画面で子テーブルの入力欄に見出しが表示される不具合修正  
    - フォーム設定のフィールド種類が「表示専用」の場合に、値が「初期値」に置き換わる不具合修正  


## v6.2.8 (2026/01/22)
1. 機能修正
    - 親子テーブルにおいて、親テーブルに閲覧権限のみを持つ場合でも、子テーブルに編集権限があれば編集できるように不具合を修正しました。
    - カスタムデータ取得APIに、親データ取得オプションを追加しました。
1. 不具合修正
    - サマリー表示でグルーピングや日付データでソートした際に順序が正しくならない不具合を修正しました。
    - 通知設定で設定したアクションがメールの場合にエラーが発生する不具合を修正しました。
    - レコード表示画面からデータ一覧画面に戻った際、元のレコード位置に戻らない不具合を修正しました。

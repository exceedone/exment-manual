...
1. 機能追加
    ...
    - カスタム列
        ...
        - 各カスタム列設定で、初期値を、列種類に応じた記入方法に対応（一部未対応はあり）
    - プラグイン
        - プラグイン(ビュー)追加。詳細は後日
        - プラグインのインストール時に、必須コンポーネントチェック機能の追加。プラグイン利用時に必須となる機能が、サーバーに含まれていない場合、インストール時にエラーを出すことができます。詳細は後日
        - プラグインのインストール時に、テンプレートのインポート機能の追加。プラグイン利用時に必須となるテンプレートを、インストール時に追加することができます。詳細は後日
    - 通知アクションで、通知先の選択肢に、「指定のメールアドレス」「システム管理者」の追加。詳細は[こちら](/ja/notify#Eメール)
    - 画面からのアップデートにプレビュー対応。詳細は[こちら](/ja/update#画面からのアップデート)
1. その他
    - 関連ライブラリlaravel-adminも含め、今後の修正時に、脆弱性が発生しにくい記述方法に修正を行いました。詳細は後日


## v4.0.6 (2021/03/08)
1. 不具合修正
    - タスクスケジューラに関する機能（データのバックアップ・通知・プラグイン）が正常に動作していない不具合の修正

...
※初期設定の場合、データ更新などを行ったログインユーザーには、通知は行われません。設定を変更する場合、[設定値変更](/ja/config)の「通知対象がログインユーザーだった場合、通知をスキップする」の設定を行ってください。
...
また、通知設定画面で、通知を行う対象を選択する「通知設定」を選択します。
...
- ##### Eメール列
データのカスタムテーブルに、カスタム列「Eメールアドレス」を含んでいた場合、そのEメールアドレスに、メールを送信します。

- ##### ユーザー列
データのカスタムテーブルに、カスタム列「ユーザー」を含んでいた場合、そのユーザー列に設定されているユーザーに、メールを送信します。
...
- ##### ユーザー列
データのカスタムテーブルに、カスタム列「ユーザー」を含んでいた場合、そのユーザーに、通知を送信します。
...
メンションを行うためのユーザーIDは、ユーザーテーブルに「Slack ID」設定用のカスタム列を追加し、その列にSlack IDを入力します。  
Slack ID設定方法は、下記の[Slack ID設定方法](#slack-id設定方法)をご確認ください。
...
- ##### ユーザー列
データのカスタムテーブルに、カスタム列「ユーザー」を含んでいた場合、そのユーザー列に設定されているSlack IDに、メンションを送信します。
...
## Slack ID設定方法
Slackで、個別メンションを実施したい場合、ユーザー列のSlack IDを入力する列設定を、はじめに行う必要があります。  
...
- ユーザーデータの編集画面で、各ユーザーのSlack IDを入力します。  

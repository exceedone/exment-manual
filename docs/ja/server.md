...
<span class="small">※かんたんインストールの権限は、インストール時のみ必要です。より厳密に権限設定する場合は、かんたんインストール実行後に、フォルダの書き込み権限を削除してください。</span>
...
## 動作環境
### サーバー
- PHP 7.2.0以上(Exment4.Xより、PHPバージョンが7.1.3以上から7.2.0以上に変更になりました)

...
2. \[PHP 7.4\] の \[VC15 x64 Non Thread Safe\] の ZIP ファイルをダウンロードします  
...
14. MySQL を利用するための管理者 (root) パスワードの初期恥を確認します  
...
12. \[規則の追加\] で \[受信規則\] の \[空の規則\] を選択して \[OK\] をクリックします  
   ![規則の追加](img/iis/iis_s17.png)
13. \[受信規則の編集\] で以下のように規則を作成します  
    ...
    - アクション
      ...
      - 書き換えられた文字列を記録する：OFF
    - 後続の規則の処理を停止する：ON
14. \[操作\] ペインで \[適用\] をクリックします
15. 「変更内容は正常に保存されました」と表示されたら \[規則に戻る\] をクリックします
...
15. IIS 実行ユーザー (IIS_IUSRS) に exment フォルダーに対する書き込みアクセス権限を追加します
...
16.  ブラウザ (Chrome または Microsoft Edge) を起動し、http://localhost/(仮想ディレクトリのエイリアス)/admin にアクセスします (例 http://localhost/exment/admin)  
...
17.  インストールが完了したら、exment フォルダーに設定していたアクセス権限を解除します
...
18.  ログファイルが書き込みできるよう、storage フォルダーに対する書き込みアクセス権限を追加します

...
v5.0.0未満では、PHP7.Xを使用しておりましたが、PHP7.4のセキュリティサポート期限が2022/11と近付いていることから、PHP8.1へと変更します。  
また、将来的なPHP・Laravelのバージョン変更を可能な限り最小限にするため、Laravelのバージョンも、現段階で最新バージョンとなるLaravel9へと変更することになりました。  

## PHPバージョンアップ方法 PHP8.1

これまでのPHPバージョンは最高でも7.4でしたが、今後はPHP8.1となります。それに伴い、すべてのユーザーが、PHP8.1へとアップデートが必要となります。  
...
## 以下の記載があれば、削除してください。(Laravel9では不要になりました)
- 削除
        "fideloper/proxy": "^4.4",
```
...
#### facade/ignitionの記載を、spatie/laravel-ignitionに変更
- 修正後

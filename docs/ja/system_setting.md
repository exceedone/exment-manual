...
## SSO設定
SSO（シングルサインオン）に関連するルールを設定します。   

![SSO設定画面](img/system_setting/system_setting_sso.png)  

#### 既定のログインを表示する
- NOにした場合、ExmentのログインページにID・パスワードを使用した既定のログインフォームが表示されません。

#### SSOログインにリダイレクト
- YESにした場合、Exmentのログインページを表示せず、SSOのログインページに直接リダイレクトします。  
※SSOの有効なプロバイダが複数件登録されている場合、この設定は無効です。

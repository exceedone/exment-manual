...
- 既定値 : true
- 役割 : trueにすると、データ検索画面の検索結果に表示される「一覧」ボタンから一覧画面を開いた場合のみ、入力した検索条件がフィルタとして適用された状態で一覧表示されます。
...
- 既定値 : false
- 役割 : trueの場合、テーブル単位のフリーワード検索時に、その時選択しているビューが維持されます。
...
#### 役割グループのインポート・エクスポートの有効化
- 設定キー : ROLE_GROUP_IMPORT_EXPORT
- 既定値 : false
- 役割 : trueにすることで、役割グループ設定画面で、データのインポート・エクスポートボタンが表示されます。  
※Exmentの権限管理について十分な知識がある方向けの拡張機能です。必ず事前にバックアップを取るなどして慎重に作業してください。
